...
import { GRASS_BLOCK_CUBE_INSTANCES, GRASS_BLOCK_CUBE_SHAPE, GRASS_BLOCK_CUBE_DIRT_STUD } from '../data/grassBlockCubes.js'
import { GRASS_BLOCK_COLORS, GRASS_BLOCK_STUD } from '../data/grassBlocks.js'
...
// Code-generated (Tech.md §7 — nothing here is downloaded), the retired
// glTF's own two-box shape (a dirt body for the lower 80%, a grass cap for
// the top 20%) rebuilt with the same bevelled two-tone stud-checker material
// GrassBlocks.jsx uses — a reference project's own NAMED block-material
// shape (see Tech.md's amendment note). Two InstancedMesh calls — one per
// material — for all 51 instances, same 2-draw-call budget the retired glTF
// cost via propModel.js's loadPropParts.
...
// One quad, UV-mapped 0..1 across its own face — same convention as
// GrassBlocks.jsx's own quad()/box(): the Y-axis side normalizes against
// [y0, y1] (that box's own height range), the X/Z side against [-half,
// half].
...
// floor / Road.jsx's road / GrassBlocks.jsx's own dirt bands and cap: owns
// its colour outright, drawn once and shared via texture.repeat rather than
// vertex-tinted.
...
// but accepts any list shaped the same way, so a second placement (e.g.
// data/pvpBlocks.js's PVP_CUBE_INSTANCES) can mount a second
// <GrassBlockCubes> without duplicating the geometry/texture-build code.
export default function GrassBlockCubes({ instances = GRASS_BLOCK_CUBE_INSTANCES }) {
  ...
  const dirtGeometry = useMemo(
    () => makeBoxGeometry(GRASS_BLOCK_CUBE_SHAPE.half, GRASS_BLOCK_CUBE_SHAPE.dirtY0, GRASS_BLOCK_CUBE_SHAPE.dirtY1),
    [],
  )
  const capGeometry = useMemo(
    () => makeBoxGeometry(GRASS_BLOCK_CUBE_SHAPE.half, GRASS_BLOCK_CUBE_SHAPE.capY0, GRASS_BLOCK_CUBE_SHAPE.capY1),
    [],
  )
  const dirtTexture = useMemo(
    () =>
      makeStudTexture(
        GRASS_BLOCK_CUBE_DIRT_STUD[0],
        GRASS_BLOCK_CUBE_DIRT_STUD[1],
        GRASS_BLOCK_STUD.dirtTileCount,
      ),
    [],
  )
  ...
  const dirtMaterial = useMemo(
    () => new THREE.MeshStandardMaterial({ map: dirtTexture, ...MATERIAL_PBR.GROUND }),
    [dirtTexture],
  )
  ...
  // and grass cap are the same 51 placements, just two different draws.
  useLayoutEffect(() => {
