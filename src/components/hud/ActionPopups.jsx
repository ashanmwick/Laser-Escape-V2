...
// easeInOutCubic — the sweep down accelerates out of the pop, eases into the
// landing.
...
// recycled — never mounted/unmounted per popup (Tech.md §5.4: the HUD must not
// re-render per frame). A self-owned rAF loop reads the pool and writes
// transform/opacity straight onto the nodes; React state is never touched here.
export default function ActionPopups() {
  const nodesRef = useRef([])

  useEffect(() => {
    let raf = 0
    // Last seq drawn into each node, so the "+N" label is written once per
    // (re)spawn rather than every frame.
    ...
    const frame = () => {
      ...
      for (let i = 0; i < ACTION_POPUP_POOL_SIZE; i++) {
        ...
        const t = slot.age / ACTION_POPUP_LIFETIME // 0..1 over the lifetime
        const pop = t < ACTION_POPUP_POP_T ? t / ACTION_POPUP_POP_T : 1
        const travel =
          t <= ACTION_POPUP_POP_T
            ? 0
            : (t - ACTION_POPUP_POP_T) / (1 - ACTION_POPUP_POP_T)
        const te = easeInOutCubic(travel)

        // Scale: spring up from small with an overshoot during the pop, then a
        // slight shrink as it sweeps away.
        let scale =
          ACTION_POPUP_POP_SCALE_FROM +
          (1 - ACTION_POPUP_POP_SCALE_FROM) * easeOutBack(pop)
        scale *= 1 - 0.18 * te

        // Position: a short upward hop during the pop (sine bump), then a lerp
        // from the frozen spawn point to the bottom-middle of the screen.
        // NDC is -1..1 with y up; convert to CSS percent (0..100, y down).
        const hop = Math.sin(pop * Math.PI) * ACTION_POPUP_HOP
        const curNdcX = slot.ndcX * (1 - ACTION_POPUP_CENTER_PULL * te)
        const curNdcY =
          slot.ndcY + (ACTION_POPUP_TARGET_Y - slot.ndcY) * te + hop
        const leftPct = (curNdcX * 0.5 + 0.5) * 100
        const topPct = (-curNdcY * 0.5 + 0.5) * 100

        // Opacity: quick fade in, hold, then fade to 0 by the end of the life.
        let opacity
        if (t < FADE_IN_T) opacity = t / FADE_IN_T
          ...
          opacity =
            1 -
            (t - ACTION_POPUP_FADE_OUT_START) / (1 - ACTION_POPUP_FADE_OUT_START)
